...
| P11-ci_workflow | `.github/workflows/container-ci.yml`, `on: pull_request` with `paths:` = the scoped dirs above (+ the workflow + CMake). **Test job (hard gate):** configure+build `wcoro_tests`, run container/algorithm suites. **Bench job:** build+run `wcontainer_bench` on PR → csv; `git checkout master`, build+run → csv (SAME runner); run P11-bench_compare; upload report + post to job summary / PR comment; fail on regression. vcpkg binary cache for gtest/nanobench/eastl. **DONE:** `.github/workflows/container-ci.yml`. **No `on.paths`** — instead 4 jobs: **`changes`** (ubuntu; `git merge-base`+`diff --name-only`, `grep -E` the scoped paths → `scoped` output), **`test`** (macos-15, if scoped; **Debug** build → run full `wcoro_tests` incl. death tests), **`bench`** (macos-15, if scoped; build+run PR bench → `pr.csv`, `git checkout origin/master` + `rm -rf build/release` + tolerant rebuild → `master.csv`, back to PR, `bench_compare.py --tol 0.15`, report → job summary + `gh pr comment`, fail on regress), **`gate`** (ubuntu, `if: always()`, needs all 3) = the **single required check**: passes when out-of-scope or when test+bench succeed, fails closed if `changes` errored. Why no `on.paths`: a path-filtered REQUIRED check never reports off-scope and blocks the PR forever — the always-running `gate` avoids that. CI **overrides** the preset's local abs-paths: `-DCMAKE_TOOLCHAIN_FILE=$VCPKG_INSTALLATION_ROOT/...` + `-DCMAKE_CXX_COMPILER=$(brew --prefix llvm)/bin/clang++`. vcpkg binary cache at `$VCPKG_DEFAULT_BINARY_CACHE`. **Verified locally:** YAML parses (ruby psych; jobs/needs/if/outputs correct); the scope regex matches all in-scope paths and ignores shared-core/math/docs/tracker. ⚠️ true end-to-end runs only on a real PR (GH Actions can't run locally). | release preset + `-DWORSE_BUILD_BENCH=ON -DVCPKG_MANIFEST_FEATURES=bench` | done |
| P11-branch_protection | Document/apply the GitHub branch-protection making the test + bench checks REQUIRED to merge into `master` — the "同意PR" gate (a repo setting, not code). **DONE (applied):** required check = the single **`gate`** context (the always-running reconciler job), `strict=false`, `enforce_admins=false` (admin can rescue), no review/push restrictions — minimal + reversible (`gh api -X DELETE …/protection`). Shipped `scripts/apply_branch_protection.sh` (idempotent PUT + read-back) and the command is also in the workflow header. **Blocker hit + resolved:** classic protection AND rulesets both 403'd on a **private repo / Free plan** ("Upgrade to Pro or make public"); user made `w6rsty/worse2026` **public**, then applied. Read-back confirms `required_status_checks.contexts:["gate"]`. ⚠️ The `gate` check only reports on PRs whose head carries the workflow — the feature/container→master PR does (workflow is on its head); PRs branched off the current bench-less master would need a rebase. | `gh api repos/w6rsty/worse2026/branches/master/protection` | done |

> **Follow-up (post-completion, tx `P11-toolchain_inject`):** de-hardcoded `CMakePresets.json` — the `default` preset's `CMAKE_TOOLCHAIN_FILE` now reads **`$env{VCPKG_ROOT}`** and `CMAKE_CXX_COMPILER` was **removed** (compiler comes from the standard `CXX`/`CC` env). Added **`scripts/toolchain/{macos.sh,ubuntu.sh,windows.ps1}`** (source-able env setup: detect brew-llvm / newest clang / LLVM + probe common vcpkg checkouts incl. `$HOME/dev/Cpp/vcpkg`; honor pre-set env; CI-aware via `$VCPKG_INSTALLATION_ROOT`; warn-not-abort; not-sourced guard) + `README.md` (usage + `CMakeUserPresets.json` alternative). **CI updated** to `source scripts/toolchain/macos.sh` then plain `cmake --preset …` (the earlier `-DCMAKE_TOOLCHAIN_FILE`/`-DCMAKE_CXX_COMPILER` overrides in the P11-ci_workflow row are now gone). Verified: scripts source-clean in bash+zsh (resolve to the exact former hardcoded paths via detection); fresh `rm -rf build/release` + `source` + configure → CMakeCache shows correct compiler+toolchain; bench rebuilds + CSV smoke (1 header/50 rows); workflow YAML re-validated; no machine paths remain in tracked build config. Local flow now: `source scripts/toolchain/<plat>` before `cmake --preset` (or use `CMakeUserPresets.json`).
