...
| P10-doc_pass | **Focused**, split per module group (contiguous / node-list / hash / ordered / algorithm / core): convert existing class/module `//` blocks → `/** \brief … */` (GAME-PERF → `\note`, keep `R##`); document top-level public APIs (`insert`/`erase`/`find`/`operator[]`/`emplace*`/`push`/`pop`/`reserve`/`sort`/`copy`/`fill`/`lowerBound`/…) with `\param`/`\tparam`/`\return`; trim genuinely verbose dev-narrative; add docs to undocumented exported APIs; skip trivial getters. | follows doxygen_setup | in_progress |
