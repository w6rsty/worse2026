...
| **Active task** | (none) |
| **Next up** | **Phase 8** in execution order (dep-sorted): 1.~~oom_predictable~~ ✓ → 2.~~hardcap_verify~~ ✓ (R46) → **3. `P8-pq_reserve` (NEXT)** → 4.fill_memset → 5.hash_forceinline → 6.swiss_tombstone → 7.ordered_transparent → 8.map_single_descent → 9.polish. Hash-cache still deferred (R40). |
...
- [ ] **Phase 8** — design-audit hardening (R45): **T1** ~~oom_predictable~~ ✓ / ~~hardcap_verify~~ ✓ (R46) · **T2** fill_memset / pq_reserve / hash_forceinline · **T3** swiss_tombstone / ordered_transparent / map_single_descent / polish *(2/9 done; debug 513/513, release 506/509 — 3 residual failures are pre-existing debug-only-assert death tests → P8-polish)*
