...
| **Active task** | (none) |
| **Next up** | **Phase 8** in execution order (dep-sorted): 1.~~oom_predictable~~ ✓ → 2.~~hardcap_verify~~ ✓ (R46) → 3.~~pq_reserve~~ ✓ → 4.~~fill_memset~~ ✓ → 5.~~hash_forceinline~~ ✓ (R47, SwissTable insert −25%) → 6.~~swiss_tombstone~~ ✓ → 7.~~ordered_transparent~~ ✓ (Set/Map default Less<>) → 8.~~map_single_descent~~ ✓ → **9. `P8-polish` (NEXT — last)**. Hash-cache still deferred (R40). |
...
- [ ] **Phase 8** — design-audit hardening (R45): **T1** ~~oom_predictable~~ ✓ / ~~hardcap_verify~~ ✓ (R46) · **T2** fill_memset / pq_reserve / hash_forceinline · **T3** swiss_tombstone / ordered_transparent / map_single_descent / polish *(8/9 done; debug 520/520, release 513/516 — 3 residual failures are pre-existing debug-only-assert death tests → P8-polish; SwissTable insert −25% via R47)*
