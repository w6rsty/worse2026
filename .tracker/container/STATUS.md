...
| **Active task** | **P7-stable_sort_buffered** (next) |
| **Next up** | **Phase 7** remaining: P7-stable_sort_buffered (~12× behind std) ← **HERE**, P7-array_trivial_push (~8×), P7-default_alloc_fastpath, P7-introsort_tune. **P7-hash_cache DEFERRED** (R40: int-key gap is structural, not hash/key-recompare; revisit with `Hash<String>`). |
...
- [ ] **Phase 7** — bench-driven perf optimizations (R39 gaps): ~~hash_cache~~ **deferred (R40)**, **stable_sort_buffered ← HERE**, array_trivial_push, default_alloc_fastpath, introsort_tune
